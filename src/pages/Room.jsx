...
import { Mic, MicOff, Video as VideoIcon, VideoOff, PhoneOff, Users, UserMinus, MonitorUp, MonitorOff, MessageSquare, Send, PanelRightOpen, PanelRightClose, Info, Copy, Check, X, ShieldCheck, ShieldX, Clock, SwitchCamera } from 'lucide-react';
...
export default function Room() {
    ...
    const assignLocalStream = useCallback((el) => {
            ...
            try {
                ...
                socket.on('all users', (usersInRoom) => {
                    ...
                    usersInRoom.forEach((userObj) => {
                        ...
                        if (streamRef.current) peer._localStreamId = streamRef.current.id;

                        // Cache remote stream to prevent React mount race condition
                        peer.on('stream', (stream) => {
                            if (streamRef.current && stream.id === streamRef.current.id) return;
                            peer._cachedRemoteStream = stream;
                        });
        ...
        <div className="fixed inset-0 bg-black flex flex-col sm:flex-row overflow-hidden pt-16 sm:pt-20">
            ...
            {(() => {
                ...
                const getMobileGridStyle = (remoteCount) => {
                    ...
                    if (remoteCount === 2) return { gridTemplateColumns: '1fr', gridTemplateRows: 'repeat(2, 1fr)' };
                ...
                // ─── MOBILE LAYOUT ───
                if (isMobile) {
                    const canSwap = peers.length === 1;
                    const showRemoteInPiP = canSwap && isPiPSwapped;
                    
                    const handlePiPClick = () => {
                        if (canSwap) setIsPiPSwapped(!isPiPSwapped);
                    };

                    const pipUserProps = showRemoteInPiP ? {
                        isVideoOff: peerMediaStatus[peers[0].peerID]?.videoOff,
                        isScreenSharing: false,
                        isMuted: peerMediaStatus[peers[0].peerID]?.muted,
                    ...
                    return (
                        <div className={`flex-1 w-full h-full pb-28 transition-all duration-300 relative`}>
                            ...
                            {peers.length > 0 && (
                                ...
                                >
                                    {showRemoteInPiP ? (
                                        <VideoStream peer={peers[0].peer} username={peers[0].username} mediaStatus={peerMediaStatus[peers[0].peerID]} />
                                    ) : (
                                        <video ref={assignLocalStream} autoPlay muted playsInline className={`w-full h-full object-cover ${isScreenSharing ? 'object-contain' : ''} ${!isScreenSharing && !isVideoOff ? '-scale-x-100' : ''}`} />
                                    )}
                                </MobilePiP>
                            )}

                            {/* Remote Videos / Swapped Main Grid */}
                            {peers.length === 0 ? (
                                // No remote peers — show Local video in grid (Zoom style)
                                <div className="w-full h-full p-2">
                                    <div className="relative bg-zinc-900 rounded-2xl overflow-hidden border border-white/10 shadow-xl flex items-center justify-center h-full w-full">
                                        {isVideoOff && (
                                            <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-zinc-900">
                                                <div className="w-16 h-16 rounded-full bg-zinc-800 flex items-center justify-center text-2xl font-bold text-white border border-white/10 mb-2">
                                        ...
                                        <video ref={assignLocalStream} autoPlay muted playsInline className={`w-full h-full object-cover ${isScreenSharing ? 'object-contain' : ''} ${!isScreenSharing && !isVideoOff ? '-scale-x-100' : ''}`} />
                                        <div className="absolute top-4 left-4 right-4 flex items-center justify-center z-20 pointer-events-none">
                                            <div className="bg-black/60 backdrop-blur-md px-4 py-2 rounded-full flex items-center gap-2 text-white text-sm font-medium border border-white/10">
                                ...
                                >
                                    {showRemoteInPiP ? (
                                        <div className="relative bg-zinc-900 rounded-2xl overflow-hidden border border-white/10 shadow-xl flex items-center justify-center h-full w-full transition-all duration-500 animate-in fade-in zoom-in-95">
                                            {isVideoOff && (
                                                <div className="absolute inset-0 flex flex-col items-center justify-center z-10 bg-zinc-900">
                                                    <div className="w-16 h-16 rounded-full bg-zinc-800 flex items-center justify-center text-2xl font-bold text-white border border-white/10 mb-2">
                                                        {(user?.username || 'A').charAt(0).toUpperCase()}
                                            ...
                                            <video ref={assignLocalStream} autoPlay muted playsInline className={`w-full h-full object-cover ${isScreenSharing ? 'object-contain' : ''} ${!isScreenSharing && !isVideoOff ? '-scale-x-100' : ''}`} />
                                            <div className="absolute bottom-3 left-3 bg-black/60 backdrop-blur-md px-2.5 py-1 rounded-lg text-xs font-medium text-white border border-white/10 flex items-center gap-2 z-20">
                                                {user?.username} (You)
                                            </div>
                                        </div>
                                    ) : (
            ...
            {/* Chat Sidebar */}
            <div className={`fixed top-0 right-0 bottom-28 sm:bottom-0 w-full sm:w-[380px] bg-zinc-950 border-l border-white/10 flex flex-col transition-transform duration-300 z-30 ${isChatOpen ? 'translate-x-0 shadow-[-20px_0_40px_rgba(0,0,0,0.5)]' : 'translate-x-full'}`}>
            ...
            <div className="fixed bottom-8 left-0 right-0 w-full flex justify-center px-4 sm:px-0 z-40 pointer-events-none">
                <div className="glass-panel px-4 sm:px-6 py-3 rounded-full flex flex-nowrap gap-4 sm:gap-6 items-center shadow-2xl border border-white/20 overflow-x-auto no-scrollbar max-w-full pointer-events-auto">
                    <button
                        onClick={toggleMute}
                        className={`shrink-0 p-3 sm:p-4 rounded-full transition-all duration-300 ${isMuted ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                    ...
                    <button
                        onClick={toggleVideo}
                        className={`shrink-0 p-3 sm:p-4 rounded-full transition-all duration-300 ${isVideoOff ? 'bg-red-500 text-white shadow-[0_0_20px_rgba(239,68,68,0.4)]' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                    ...
                    <button
                        ...
                        className={`shrink-0 p-3 sm:p-4 rounded-full transition-all duration-300 ${isScreenSharing ? 'bg-blue-500 text-white shadow-[0_0_20px_rgba(59,130,246,0.4)]' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                    ...
                    <button
                        ...
                        className="shrink-0 sm:hidden p-3 rounded-full transition-all duration-300 bg-white/10 hover:bg-white/20 text-white"
                        title="Flip camera"
                    >
                    ...
                    <button
                        ...
                        className={`shrink-0 p-3 sm:p-4 rounded-full transition-all duration-300 relative ${isParticipantsOpen ? 'bg-white text-black' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                        title="Participants"
                    >
                        <Users size={22} />
                    ...
                    <button
                        ...
                        className={`shrink-0 p-3 sm:p-4 rounded-full transition-all duration-300 ${isChatOpen ? 'bg-white text-black shadow-[0_0_20px_rgba(255,255,255,0.2)]' : 'bg-white/10 hover:bg-white/20 text-white'}`}
                        title="Toggle Chat"
                    >
                    ...
                    <button
                        ...
                        className="shrink-0 bg-white/10 hover:bg-white/20 text-white p-3 sm:p-4 rounded-full transition-all duration-300 relative"
                        title="Invite via Link"
                    >
                        <Info size={22} />
                    </button>

                    <div className="shrink-0 w-px h-8 bg-white/20 mx-1 sm:mx-2" />

                    <button
                        onClick={isHost ? endMeeting : leaveRoom}
                        className="shrink-0 bg-red-600 hover:bg-red-500 text-white p-3 sm:p-4 rounded-full transition-all duration-300 shadow-[0_0_30px_rgba(220,38,38,0.5)] hover:scale-105"
            ...
            <div className={`fixed right-0 top-0 bottom-28 sm:bottom-0 w-full sm:w-80 bg-zinc-950 border-l border-white/10 z-[60] flex flex-col transition-transform duration-300 ${isParticipantsOpen ? 'translate-x-0' : 'translate-x-full'}`}>
                <div className="p-6 border-b border-white/10 flex items-center justify-between">
                    ...
                    <button onClick={() => setIsParticipantsOpen(false)} className="text-zinc-400 hover:text-white hidden sm:inline-flex">
                        <X size={24} />
                    </button>
                </div>
...
const VideoStream = ({ peer, username, mediaStatus }) => {
    ...
    useEffect(() => {
        // Use the cached stream if it was received before mount
        if (peer._cachedRemoteStream) {
            setRemoteStream(peer._cachedRemoteStream);
        }

        const handleStream = (stream) => {
            if (peer._localStreamId && stream.id === peer._localStreamId) return;
            peer._cachedRemoteStream = stream;
...
const MobilePiP = ({ isVideoOff, isScreenSharing, isMuted, username, onClick, children }) => {
    ...
    useEffect(() => {
        if (position.x === -1 && position.y === -1) {
            const safeX = window.innerWidth - 90 - 12; // 90 = pip width
            setPosition({ x: safeX, y: 72 });
        }
    }, []);
    ...
    const handleTouchMove = useCallback((e) => {
        ...
        const maxY = window.innerHeight - 160 - 110; // 160 = pip height, 110 = dock clearance
        setPosition({
    ...
    const handleTouchEnd = useCallback((e) => {
        ...
        const dist = Math.abs(touch.clientX - dragState.current.initialX) + Math.abs(touch.clientY - dragState.current.initialY);
        if (dist < 10 && onClick) onClick();
    }, [onClick]);
    ...
    const handleMouseDown = useCallback((e) => {
        ...
        const handleMouseMove = (ev) => {
            ...
            const maxX = window.innerWidth - 90;
            const maxY = window.innerHeight - 160 - 110;
            setPosition({
                x: Math.max(4, Math.min(newX, maxX)),
                y: Math.max(4, Math.min(newY, maxY))
            });
        ...
        const handleMouseUp = (ev) => {
            dragState.current.isDragging = false;
            setIsDragging(false);
            const dist = Math.abs(ev.clientX - dragState.current.initialX) + Math.abs(ev.clientY - dragState.current.initialY);
            if (dist < 10 && onClick) onClick();
